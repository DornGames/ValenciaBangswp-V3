...
```
/
├── index.html                  # 首页 — 跳转到 src/index.html
├── src/
│   ├── index.html              # 首页 — 搜索入口
│   ├── css/
│   │   ├── main.css            # 全局样式表入口（@import 汇总）
│   │   ├── base.css            # 基础层：CSS 变量、Google Fonts、全局重置、基础元素样式、工具类
│   │   ├── layout.css          # 布局层：导航栏、页脚、容器布局、分割线
│   │   ├── components.css      # 组件层：搜索框、单词卡片、详情区、翻译组件、详情表格、动画
│   │   └── pages.css           # 页面层：背景图片、错误图片、搜索页专属样式
│   ├── js/
│   │   ├── tools.js            # 工具函数（字母转换、莱文斯坦距离、单词派生）
│   │   ├── wordData.js         # 词汇数据
│   │   ├── exampleData.js      # 例句库数据（10个语法示例）
│   │   ├── exampleUtils.js     # 例句匹配算法（分词推导、反向查找、例句匹配）
...
### 2026-07-31 — 第二十三阶段：CSS 模块化拆分
- **重构** `src/css/main.css` — 将单一 CSS 文件拆分为模块化多文件：
  - `src/css/main.css` — 改为 @import 汇总入口，按顺序引入 base → layout → components → pages
  - `src/css/base.css`（新增）— 基础层：CSS 变量（:root）、Google Fonts 引入、全局重置样式、body/html/a/button/img/ul 基础样式、滚动条隐藏、工具类
  - `src/css/layout.css`（新增）— 布局层：导航栏（#nav, #navLogo, #navMar, 汉堡菜单）、页脚（#bottom_div, #copyright）、容器布局（.container）、分割线（.divider）、响应式中导航/页脚相关部分
  - `src/css/components.css`（新增）— 组件层：搜索框（.login-wrapper, .form-wrapper, #selectLan, .input-item 等）、搜索下拉（#search-list）、Logo 区域（#logoContainer）、单词卡片（.word-card）、详情区（.intro-wrapper）、翻译组件（#leftTA, #translateBtn, #translateResult）、详情表格（.detail-table）、工具列表（.tools）、iframe（#alphabet）、动画（@keyframes）、响应式中组件相关部分
  - `src/css/pages.css`（新增）— 页面层：背景图片（#bg）、错误图片（#errorImg）、搜索页专属样式
- 拆分规则：CSS 变量在 base.css 中定义，其他文件通过变量引用；响应式 @media 按主要作用模块拆分；动画 @keyframes 放在 components.css

### 2026-07-31 — 第二十四阶段：项目结构全量重构 + Bug 修复
- **重构** 项目目录结构 — 全面重新规划文件组织：
...
- **新增** `src/js/components.js` — 共享组件模块，提供 `initNav(isSubpage)` 和 `initFooter()` 统一管理导航栏和页脚
- **重构** `src/js/main.js` — 使用 `initNav(false)` 和 `initFooter()` 替代内联事件和 `copyright()` 函数
- **重构** `src/index.html` — 移除所有导航按钮的 `onclick` 属性，通过 `components.js` 统一管理
- **重构** `src/pages/alphabet.html`、`bwbæzencl.html`、`tondhæ.html`、`edusrc.html` — 移除导航按钮 `onclick`，改用 `initNav(true)` + `initFooter()`
- **修复** `src/pages/translation.html` — `window.open('','_self')` → `window.alert('正在开发,敬请期待')`（翻译按钮刷新页面Bug）
- **修复** `src/pages/translation.html` — `new setMultiWord().setAllWord()` → `setMultiWord.setAllWord()`（`setMultiWord` 已是实例，非构造函数）
- **修复** `src/pages/edusrc.html` — `copyright()` → `initNav(true); initFooter()`（词汇量链接不显示）
- **修复** `src/js/components.js` — `initNav()` 路径逻辑：`basePath` 字符串拼接改为 `isSubpage` 布尔参数，正确区分首页(`pages/`前缀)和子页面(同级路径)
- **更新** `.github/workflows/deploy.yml` — 添加构建步骤，将 `src/` 和 `public/` 合并到 `dist/` 目录再部署
- **新增** `index.html`（根目录）— 自动跳转到 `src/index.html` 的入口页
- **更新** `docs/README.md` — 项目结构更新
- **更新** `z/projectdetail.md` — 项目结构更新

### 2026-07-31 — 第二十五阶段：修复搜索功能无法查词
- **修复** `src/pages/search.html` — 搜索功能异常问题全面修复：
  - 在 `<head>` 中添加 `<script src="../js/components.js"></script>` 加载，确保页脚组件正常初始化
  - 用 try-catch 包裹 `setMultiWord.setAllWord()` 和 `copyright(true)`，防止派生词生成出错阻塞后续代码
  - 处理 URL 无 `s` 参数时 JSON.parse 报错，首次访问搜索页不再崩溃
  - 声明未声明的变量 `type`，移除未使用的 `searchType`
  - 无匹配单词时显示"未找到该单词"友好提示，避免访问 undefined 属性报错
  - 添加全局 try-catch 保护，确保参数异常时显示友好错误信息

### 2026-07-31 — 第二十六阶段：新增例句库数据文件 + 搜索结果页展示例句
- **新增** `src/js/exampleData.js` — 创建例句库数据文件，包含10个符合语法规则的湩语例句：
  - 涵盖现在时、过去时、将来时三种时态
  - 包含简单句和逗号+hr连接的略复杂句
  - 使用分词形态（现在分词、过去分词、被动分词）
  - 使用使动分词（祈使句）
  - 使用副词修饰动词
  - 使用疑问句（wna引出）
  - 所有词汇均来自 wordData.js

### 2026-07-31 — 第二十六阶段：搜索结果页展示例句
- **修改** `src/pages/search.html` — 在搜索结果页的单词详情下方添加例句展示区域：
  - 在 `<head>` 中添加 `<script src="../js/exampleData.js"></script>` 引用
  - 新增 `.example-section` 卡片样式（与现有 `.word-card` 风格一致）
  - 在 `.word-card` 下方添加例句展示区域，显示湩语原文和中文翻译
  - 实现 `getOriginalDh` 函数，将分词形式还原到原动词词根
  - 实现 `showExamples` 函数，按关联规则查找并展示例句
  - 关联规则：直接匹配（dh 在 words 数组中）、分词关联（分词→原动词）、原动词关联（动词→所有分词形式）
  - 无例句时显示"暂无例句"占位提示
  - 支持响应式布局适配

### 2026-07-31 — 第二十七阶段：改进 tondhæDetail.html 为动态单词详情页
- **改造** `src/pages/tondhæDetail.html` — 从静态时态表格全面改造为动态单词详情展示页：
  - 通过 URL 参数 `s` 接收单词，展示拼写（拉丁+湩语字母）、词性、中文释义
  - 从 `exampleData` 中查找相关例句，支持分词关联查找（7种分词类型：过去/现在/将来/被动/使动/意动/为动）
  - 分词单词自动推导原动词，原动词自动查找所有分词形式的例句
  - 有例句展示例句列表，无例句显示"暂无例句"
  - 无 `s` 参数时显示宁静回退提示
  - 加载 `exampleData.js`、`components.js`，使用项目统一样式（暖褐配色）
  - 分词单词显示原动词来源信息

### 2026-07-31 — 第二十九阶段：例句高亮显示 + 例句库扩充至40句
- **新增** `src/js/exampleUtils.js` — 新增 `getWordMatchForms` 和 `highlightSentence` 函数，实现例句中相关词汇（单词自身、分词形式、原动词）的自动高亮
- **扩充** `src/js/exampleData.js` — 例句从10句扩充至40句，新增30个涵盖不同语法点的例句：
  - 基础时态：现在时、过去时、将来时
  - 被动语态：aizêr + 被动分词
  - 祈使句：cmar + ki-使动分词
  - 疑问句：wna + 主语 + 谓语
  ...
  - 连词：hr 连接宾语或分句
  - 分词作定语：现在分词 + 名词
  - 介动词：ich（在...里）、opdên（在...上）
  - 否定句：sht 否定
  - 所有格、指示代词等
- **修改** `src/pages/tondhæDetail.html` — 例句渲染使用 `highlightSentence` 高亮相关词汇；新增 `.example-highlight` CSS 样式（红色粗体+浅色背景）
- **修改** `src/pages/search.html` — 同上，例句渲染使用 `highlightSentence` 高亮相关词汇；新增 `.example-highlight` CSS 样式

### 2026-07-31 — 第三十阶段：例句库扩充至90句（新增50句）
- **扩充** `src/js/exampleData.js` — 例句从40句扩充至90句，新增50个涵盖更多语法点和词汇的例句：
  - **第八组：更多动词的现在时（5句）** — 使用新动词：haor（想）、sarbib（喜爱）、blan（玩）、neu（学习）、zipwshiêt（跑步）
  - **第九组：不同动词的过去时（5句）** — 使用新动词：apui（说）、plon（思考）、zœb（完成）、drên（发现）、glwxtod（创作）
  - **第十组：不同动词的将来时（5句）** — 使用新动词：klemt（拜访）、glwnt（朗诵）、strok（旅行）、kriwhiêt（介绍）、krwêsh（推荐）
  - **第十一组：被动语态（5句）** — 使用新受事/施事：dronch（门）、frinch（窗）、banpui（歌曲）、epsh（水），新动词：shlanzod（打开）、klêmpod（关闭）
  - **第十二组：祈使句与情态词（5句）** — 新增情态词：hreh（应该）、kiah（可以）；祈使句：kizanod（坐下）、kineu（学习）
  - **第十三组：疑问句（特殊疑问词，5句）** — 使用特殊疑问词：tah（什么）、wni（谁）、wadheo（为什么）、wno（何时）、woh（如何）
  - **第十四组：连词与复合句（5句）** — 使用连词：hinhh（因为）、hanhh（所以）、hihh（如果）、whr（也）、hohh（但是）
  - **第十五组：否定句与所有格（5句）** — 使用新词汇：valæhiêt（懂得）、sorch（拥有）、plirp（钱）、madfak/caofak（所有格）
  - **第十六组：介动词与地点（5句）** — 使用介动词：opdên（在...上）、apdên（在地面上）、nih（与...一起）、zanz（随着）
  - **第十七组：比较级与最高级（5句）** — 使用比较级副词：nesh（更）、loht（一样）、neshn（最）、zind（更少）
- **扩充** `src/js/exampleData.js` — 文件头注释更新为90句，新增第三十阶段更新日志

### 2026-07-31 — 第二十八阶段：例句匹配算法优化 + 分词自动查找 + 搜索页面路径修复
- **优化** `src/js/exampleUtils.js` — 新增 `findOriginalVerbByParticiple` 函数，通过遍历 `wordData` 中所有动词并生成其分词形式，实现分词→原动词的反向查找
- **优化** `src/js/exampleData.js` — 移除冗余的 `words` 数组，仅保留 `dh` 和 `cn` 字段，例句与单词的关联通过 `exampleUtils.js` 算法自动匹配
- **新增** `src/js/exampleUtils.js`（上一会话创建，本次新增算法）— 分词推导、例句匹配、反向查找等核心工具函数集合
- **修复** `src/pages/tondhæDetail.html` — 当搜索的单词不在 `wordData` 中时，自动通过 `findOriginalVerbByParticiple` 反向查找分词的原动词，并显示"您查找的是...属于动词...的分词形式"提示；未找到时显示"未找到该单词"友好提示
- **修复** `src/pages/search.html` — 同上，支持分词自动映射到原动词；修复 `search.html` 中首页搜索结果跳转路径（`../pages/search.html` → `pages/search.html`）
- **修复** `src/js/main.js` — 修复搜索跳转路径错误（`../pages/search.html` → `pages/search.html`）

### 2026-07-31 — 第三十阶段：词汇扩充（新增 20 动词 + 40 名词）
- **扩充** `src/js/wordData.js` — 新增 80 个词条（20动词+20名词派生+40独立名词，严格审查无重复）：
  - **动词（20个，无词缀 → 有词缀名词 -iat，词缀多样化）**：
    - `glwng` 打嗝,呃逆,嗝 → `glwngiat` 打嗝,呃逆,嗝声
    - `glwft` 摘,采摘,摘取 → `glwftiat` 采摘,摘取,摘采
    - `brwsk` 挑,挑选,拣选 → `brwskiat` 挑选,拣选,选择
    - `drwsk` 甩,甩动,甩脱 → `drwskiat` 甩动,甩脱,甩弃
    - `krwft` 冒,冒出,涌现 → `krwftiat` 冒出,涌现,涌出
    - `vlwst` 咳,咳痰,咯痰 → `vlwstiat` 咳痰,咯痰,痰
    - `vlwnd` 吐,呕吐,吐呕 → `vlwndiat` 呕吐,吐呕,吐秽
    - `zlwnd` 打哈欠,呵欠,欠伸 → `zlwndiat` 哈欠,呵欠,欠伸
    - `flwnd` 擤,擤鼻,醒鼻 → `flwndiat` 擤鼻,醒鼻,鼻涕
    - `drwnd` 打喷嚏,喷嚏,嚏 → `drwndiat` 喷嚏,嚏,喷鼻
    - `srwnd` 伸,伸展,舒展 → `srwndiat` 伸展,舒展,伸张
    - `zlwmp` 跺,跺脚,顿足 → `zlwmpiat` 跺脚,顿足,跺地
    - `srwmp` 蹲,蹲下,蹲坐 → `srwmpiat` 蹲下,蹲坐,蹲姿
    - `vlwsk` 跪,跪拜,跪伏 → `vlwskiat` 跪拜,跪伏,跪姿
    - `zlwsk` 趴,趴伏,俯卧 → `zlwskiat` 趴伏,俯卧,趴姿
    - `grwsk` 插,插入,插进 → `grwskiat` 插入,插进,插孔
    - `prwsk` 拔,拔出,拔除 → `prwskiat` 拔出,拔除,拔取
    - `blwft` 洒,洒水,喷洒 → `blwftiat` 洒水,喷洒,洒落
    - `vlwft` 拂,拂拭,拂去 → `vlwftiat` 拂拭,拂去,拂尘
    - `flwmp` 呸,吐唾沫,啐 → `flwmpiat` 唾沫,啐唾,吐沫
  - **名词（40个，独立名词，词根多样化）**：
    - 自然气象：`blwsk` 闪电 — `srwsk` 冰雹 — `srwft` 露珠 — `trwft` 薄雾 — `flwft` 尘雾 — `slwft` 气泡 — `vlwmp` 沙子 — `stwmp` 田野 — `vlwng` 微风 — `flwng` 暴风 — `stwng` 旋风
    - 身体部位：`vlwnt` 脚 — `zlwnt` 手臂 — `srwnt` 胸部 — `grwnt` 腹部 — `stwnt` 臀部 — `brwnt` 脚趾 — `trwnt` 指尖 — `drwnt` 脚跟 — `frwnt` 脚背 — `krwnt` 腰部 — `slwnt` 脚掌
    - 抽象概念：`blwrn` 意义 — `vlwrn` 价值 — `zlwrn` 命运 — `srwrn` 信仰 — `grwrn` 希望 — `prwrn` 梦想 — `brwrn` 原则 — `trwrn` 标准 — `drwrn` 目标 — `frwrn` 象征 — `krwrn` 比喻 — `slwrn` 节奏 — `klwrn` 旋律 — `plwrn` 和谐 — `glwrn` 平衡 — `flwrn` 稳定 — `stwrn` 永恒 — `blwrp` 瞬间

### 2026-07-31 — 第二十九阶段：词汇扩充（新增 20 动词 + 35 名词）
- **扩充** `src/js/wordData.js` — 新增 55 个词条（20动词+20名词派生+35独立名词，严格审查无重复）：
  - **动词（20个，无词缀 → 有词缀名词 -iat/-iad，词缀多样化）**：
    - `zlopt` 舔,舔舐,舔食 → `zloptiat` 舔舐,舔食,舔痕
    - `krimb` 拧,扭转,拧紧 → `krimbiat` 拧转,扭转,拧紧
    - `blund` 吞,吞咽,咽下 → `blundiat` 吞咽,咽下,吞食
    - `zrank` 责备,指责,斥责 → `zrankiad` 责备,指责,斥责
    - `zrwnd` 咳嗽,咳嗽,咳声 → `zrwndiat` 咳嗽,咳声,咳嗽
    - `strend` 滑,滑动,滑行 → `strendiat` 滑动,滑行,滑移
    - `grêst` 欠,亏欠,拖欠 → `grêstiat` 欠债,亏欠,拖欠
    - `mrend` 量,测量,计量 → `mrendiat` 测量,计量,度量
    - `drwng` 晾,风干,晾干 → `drwngiad` 晾晒,风干,晾干
    - `zrênp` 皱,皱眉,皱起 → `zrênpiad` 皱纹,皱褶,皱痕
    - `zlink` 眨,眨眼,眯眼 → `zlinkiat` 眨眼,眯眼,眨动
    - `plwnd` 还,归还,返还 → `plwndiat` 归还,返还,偿还
    - `zrœnd` 滚,滚动,翻滚 → `zrœndiat` 滚动,翻滚,滚转
    - `glwsk` 吸,吸气,吸入 → `glwskiad` 吸气,吸入,吸力
    - `brwng` 绷,绷紧,绷直 → `brwngiad` 绷紧,绷直,绷张
    - `klwng` 刻,雕刻,镌刻 → `klwngiat` 雕刻,镌刻,刻痕
    - `trwng` 翻,翻转,翻身 → `trwngiad` 翻转,翻转,翻动
    - `grwng` 钩,钩取,钩挂 → `grwngiat` 钩取,钩挂,钩子
    - `plwng` 碰,碰撞,碰触 → `plwngiat` 碰撞,碰触,撞击
    - `flwrz` 渗,渗透,渗入 → `flwrziat` 渗透,渗入,渗漏
  - **名词（35个，独立名词，词根多样化）**：
    - `zrump` 耳朵,耳,耳廓 — `nwmp` 鼻子,鼻,鼻腔 — `zlang` 舌头,舌,舌尖
    - `znwmp` 指甲,趾甲,指甲 — `mœnd` 月亮,月球,月色 — `zremp` 沙漠,荒漠,沙海
    - `klwsk` 岛屿,岛,海岛 — `zrœsp` 叉子,叉,餐叉 — `zrœp` 绳子,绳索,绳
    - `zrœng` 性别,性别,性 — `zrœrk` 山坡,坡地,斜坡 — `krwnd` 角落,角落,隅
    - `krwst` 门槛,门槛,门限 — `grwnd` 底部,底部,底 — `slwng` 标题,题目,名目
    - `zlwng` 期限,时限,定期 — `zrwng` 档次,等级,品级 — `prwng` 条目,项目,款项
    - `zrœst` 轮廓,外形,概貌 — `zrwst` 手腕,腕,腕部 — `zrœmp` 脚踝,踝,踝部
    - `drwst` 背,背部,脊背 — `zrœmf` 额头,额,前额 — `zrœsg` 小腿,腿肚,胫
    - `zrœft` 肘,肘部,肘弯 — `plwft` 波浪,浪,波涛 — `prwft` 雷,雷声,雷鸣
    - `strwft` 彩虹,虹,彩练 — `frwng` 宽度,阔度,幅 — `vrwst` 深度,深浅,深
    - `srwst` 高度,高矮,高 — `zrwft` 顺序,次序,序次 — `zrœvr` 手掌,掌心,掌
    - `zrœfth` 大腿,股,腿部 — `zrœnt` 下巴,下颌,颔
...
### 2026-07-31 — 第三十阶段：版本升级至 V3.1
- **版本更新** 将项目版本从 3.0.0 升级至 V3.1
- **重写 README.md** 全面更新项目说明文档
- **更新 intro.md** 同步版本号

### 2026-07-31 — 第三十一阶段：全量版本号统一更新至 V3.1
- **修改** `src/index.html` 第69行 — 页脚版本号 `Ver 3.0.0` → `Ver V3.1`
- **修改** `src/js/components.js` 第61行 — 动态页脚版本号 `Ver 3.0.0` → `Ver V3.1`
- **修改** `src/pages/search.html` 第156行 — 页脚版本号 `Ver 2.0.0` → `Ver V3.1`
- **修改** `src/pages/tondhæDetail.html` 第208行 — 页脚版本号 `Ver 2.0.0` → `Ver V3.1`